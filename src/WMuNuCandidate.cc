...
// WMuNuCandidate::WMuNuCandidate(edm::Ptr<reco::PFCandidateCollection> muon, edm::Ptr<reco::PFMET> met): muon_(muon), neutrino_(met)  
...
double WMuNuCandidate::massT() const{
  ...
  mt = (mt>0) ? sqrt(mt) : 0;
  std::cout<<"INSIDE W candidate px "<<wpx<<" wpy "<<wpy<<" wpx  "<<wpx<<std::endl;
  return mt; 
}
